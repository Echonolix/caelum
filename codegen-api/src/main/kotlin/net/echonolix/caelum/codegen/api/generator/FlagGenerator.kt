...
public open class FlagGenerator(ctx: CodegenContext, element: CType.EnumBase) : EnumBaseGenerator(ctx, element) {
    ...
    override fun buildCompanionType(): TypeSpec.Builder {
        ...
        constEntries.forEach {
            val expression = it.expression as CExpression.Const
            val initilizer = CodeBlock.of("%T(%L)", thisCName, expression.codeBlock())
        ...
        aliasEntries.forEach {
            val expression =  it.expression as CExpression.Reference

            assert(expression.value in entries)
            val initilizer = CodeBlock.of("%N", expression.value.tags.get<EnumEntryFixedName>().name)
